...
export class TrendsComponent implements OnInit {
  ...
  periodList = ["Last30Min" , "Last1Hr" , "Last5Hr" , "Last1Day" , "Last1Week" , "Last1Month"];
  lineData = 
    {
      'data' : [],
      'label' : '' 
 ...
 datasets = [{
    ...
    this.restService.getServerUtilizationStats(params).then(res => {
      if (res) {
        if (res['status'] === 200) {
		  ...
		  for(let i=0 ; i< this.dataService.serverUtilization.memoryData.processingTime.length; i++){
		  console.log("pushing data");
		   this.datasets[0].data.push({
		   
		    x: this.dataService.serverUtilization.memoryData.processingTime[i],
  ...
  setClickedRow(index){
    ...
    selectdate.setMinutes(selectdate.getMinutes() + 60);
	  ...
	  let selecteddate=this.datepipe.transform(selectdate, 'yyyy-MM-ddTHH:mm:ssZ');
      ...
      if (res) {
        if (res['status'] === 200) {
          ...
          for(let i=0; i<responseData.length ; i++){
            ...
            this.notifications.push(responseData[i][1]+": CPU/Mem spike at "+responseData[i][2])
          }
...
const activePoints = chart.getElementAtEvent(e.event);
  ...
  const value = chart.data.datasets[0].data[clickedElementIndex];
	...
	ydaydate.setMinutes(ydaydate.getMinutes() - 60);
	let ydaydat=this.datepipe.transform(ydaydate, 'yyyy-MM-ddTHH:mm:ssZ');
	let selecteddate=this.datepipe.transform(new Date(value.x), 'yyyy-MM-ddTHH:mm:ssZ');
	this.getCriticalLogs(selecteddate,ydaydat)	
